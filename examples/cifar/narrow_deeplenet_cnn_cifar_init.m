function net = narrow_deeplenet_cnn_cifar_init(varargin)
...
% Block 1
net = add_conv(net, [5 5 3 16], lr, 1, 2, nnz(1));
...
% Block 2
net = add_conv_custom(net, [5 5 16 16], lr, 1, 2, nnz(2), useGPU);
net.layers{end}.weights{1}=randn(5, 5, 16, 16, 'single')*0.001;
net.layers{end+1} = struct('type', 'relu') ;
net = add_conv_custom(net, [5 5 16 16], lr, 1, 2, nnz(3), useGPU);
net.layers{end}.weights{1}=randn(5, 5, 16, 16, 'single')*0.001;
net.layers{end+1} = struct('type', 'relu') ;
net = add_conv_custom(net, [5 5 16 16], lr, 1, 2, nnz(4), useGPU);
net.layers{end}.weights{1}=randn(5, 5, 16, 16, 'single')*0.001;
net.layers{end+1} = struct('type', 'relu') ;
...
% Block 3
net = add_conv_custom(net, [5 5 16 32], lr, 1, 2, nnz(5), useGPU);
net.layers{end}.weights{1}=randn(5, 5, 16, 32,  'single')*0.001;
net.layers{end+1} = struct('type', 'relu') ;
net = add_pool(net, 'avg', [3 3], 2, [0 1 0 1]);

% Block 4
net = add_conv_custom(net, [4 4 32 32], lr, 1, [1,2,1,2], nnz(6), useGPU);
net.layers{end}.weights{1}=randn(4, 4, 32, 32, 'single')*0.01;
net.layers{end+1} = struct('type', 'relu') ;
net = add_conv_custom(net, [4 4 32 64], lr, 1, [1,2,1,2], nnz(7), useGPU);
net.layers{end}.weights{1}=randn(4, 4, 32, 32, 'single')*0.01;
net.layers{end+1} = struct('type', 'relu') ;
